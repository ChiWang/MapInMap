 ...
 *----------------------------------------------------------
 *  Setup install method for mapInMap
 *  1. if you want to install the whole mapInMap, execute
 *      scons
...
#--------------------------------------------------------------------
project="mapInMap"    # or you can set an ABBREVIATION
prefix=project+'Install'
'''
if os.environ.has_key(project+'SYS'):
    prefix=os.environ[project+'SYS']
else:
    UsrPrefix=raw_input('Where to install mapInMap (press Enter to use default "%s"): ' %prefix)
    UsrPrefix=UsrPrefix.strip()
    if UsrPrefix:
        prefix=UsrPrefix
'''
prefix=os.path.abspath(prefix)
print "install mapInMap into: ", prefix
